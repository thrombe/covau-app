<script lang="ts">
    ...
    const is_selected = (index: number, selected: number) => {
        ...
        return (
            selected == i || (i == items.length - 1 && selected >= items.length)
        );
    };
...
<sbp class="flex flex-row h-full w-full">
    ...
    >
        ...
        >
            <Observer
                enter_screen={end_reached}
                bind:visible={end_is_visible}
                {root}
                {margin}
            />
        </obs>
